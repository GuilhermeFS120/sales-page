...
function setFullScreen() {
    if (!document.webkitFullscreenElement) {
        ...
        videoFullScreen
            .querySelector('i.bx')
            .classList.add('bx-exit-fullscreen')
    } else {
        document.webkitExitFullscreen()
        videoFullScreen
            .querySelector('i.bx')
            .classList.remove('bx-exit-fullscreen')
...
function seeker(event) {
    let mousePos =
            event.clientX - (document.body.clientWidth - vsl.clientWidth) / 2,
...
vsl.addEventListener('mousemove', () => {
    if (video.paused) {
        showControls()
    }
})
vsl.addEventListener('mouseout', () => {
    hideControls()
    if (video.paused) {
